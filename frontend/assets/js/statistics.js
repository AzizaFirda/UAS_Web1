...
const Statistics = {
    ...
    updateOverviewCards: function(stats) {
        ...
        if (totalBalanceEl) {
            ...
            totalBalanceEl.className = 'mb-0 ' + (balance >= 0 ? 'text-success' : 'text-danger');
        }
    ...
    displayCategoryList: function(categories) {
        ...
        if (!categories || categories.length === 0) {
            container.innerHTML = '<p class="text-center text-muted">Belum ada data pengeluaran</p>';
            return;
        }
    ...
    loadAccountStats: async function() {
        try {
            const response = await fetch(`${APP_CONFIG.API_URL}/statistics.php?action=account`, {
                credentials: 'include'
            });
            const data = await response.json();
    ...
    displayAccountList: function(accounts) {
        ...
        if (!accounts || accounts.length === 0) {
            container.innerHTML = '<p class="text-center text-muted">Belum ada akun</p>';
            return;
        }
    ...
    loadTrendData: async function() {
        try {
            const response = await fetch(`${APP_CONFIG.API_URL}/statistics.php?action=trend&months=12`, {
                credentials: 'include'
            });
            const data = await response.json();
...
async function exportReport(format) {
    try {
        ...
        const response = await fetch(`${APP_CONFIG.API_URL}/export.php?format=${format}&month=${month}&year=${year}`, {
            credentials: 'include'
        });
